        ...
        with st.form("reg_doc", clear_on_submit=True):
            ...
            if st.form_submit_button("✅ Register Doctor", type="primary", use_container_width=True):
                ...
                if errors:
                    ...
                    # Step 1: Create login account (role=doctor, pending approval via is_active=False)
                    auth_payload = {
                        "email": d_email.strip(),
                        "password": d_pwd,
                    ...
                    if auth_resp and auth_resp.status_code == 201:
                        ...
                        if doc_code == 201:
                            # Step 3: Auto-activate since admin is registering the doctor
                            try:
                                requests.patch(f"{API_URL}/admin/activate",
                                               params={"email": d_email.strip()},
                                               headers=_headers(), timeout=8)
                            except: pass
                            st.success(f"✅ Dr. {d_fn} {d_ln} registered & activated!")
                            st.info(f"🔐 Login credentials:\n\n**Email:** {d_email.strip()}\n\n**Password:** (the one you just set)")
                            st.balloons()
                        else:
                            st.warning(f"⚠️ Login account created but profile failed: {doc_data.get('detail','Error')}. Doctor can log in but profile is incomplete.")
                    elif auth_resp:
            ...
            if pending_resp.status_code == 200:
                ...
                if not pending_doctors:
                    ...
                    for pu in pending_doctors:
                        ...
                        with col_btn:
                            if st.button(f"✅ Approve", key=f"approve_doc_{pu['id']}"):
                                try:
